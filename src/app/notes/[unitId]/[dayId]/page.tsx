...
export async function generateMetadata({ params }: PageProps) {
  ...
  if (!content || !unitInfo) {
    return {
      title: "Lesson Not Found | LearnWithJulfy",
      description: "The requested education lesson was not found."
    };
  }

  const dayNumber = parseInt(dayId.replace("day", ""), 10);
  const title = `Class 12 Education AHSEC Unit ${unitInfo.numberKey} Day ${dayNumber} Notes | LearnWithJulfy`;
  const description = `Learn about ${content.english.title} (${content.assamese.title}) in this daily lesson. Study concepts, attempt practice MCQs, check board revision tips, and take timed mini tests.`;
...
export default function DayNote({ params }: PageProps) {
  const { unitId, dayId } = params;
  const unitInfo = unitMap[unitId];
